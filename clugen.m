...
% CLUGEN Generates n-dimensional data for clustering. Data is created
...
addRequired(p, 'totalPoints', ...
    @(x) isnumeric(x) && isscalar(x) && (x > 0) && (mod(x, 1) == 0));
addRequired(p, 'dirMean', @isnumeric);
addRequired(p, 'dirStd', @isnumeric);
...
parse(p, ndim, numClusts, totalPoints, dirMean, dirStd, clustSepMean, ...
